    ...
    *initClass {
    ...
    addSynthdefs {
        SynthDef(\fxTape, { |inBus, outBus|
            ...
            // --- GENERACIÓN ESTÉREO POST-CINTA (Tape Skew Bipolar & Crosstalk) ---
            // LFO de cabeceo: oscila entre -0.75ms y +0.75ms
            skew_lfo = LFNoise2.kr(0.1).range(-0.00075, 0.00075);
            
            // Retraso base de 0.75ms para permitir modulación bipolar invertida
            skew_l = DelayC.ar(final_mono, 0.01, 0.00075 + skew_lfo);
            skew_r = DelayC.ar(final_mono, 0.01, 0.00075 - skew_lfo);

            // Diafonía Magnética (Crosstalk del 5% entre pistas)
